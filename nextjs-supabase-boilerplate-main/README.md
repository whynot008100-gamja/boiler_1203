...
**3-2. Supabase Dashboard에서 Clerk Third-Party Auth Provider 추가**
...
설정이 완료되면 Clerk 세션 토큰이 자동으로 `role: 'authenticated'` 클레임을 포함하게 되고, Supabase가 이를 검증할 수 있게 됩니다.

자세한 내용은 [통합 가이드 문서](./docs/CLERK_SUPABASE_INTEGRATION.md)를 참조하세요.
...
3. 다음 순서로 마이그레이션 실행:
   ...
   **5-1. `setup_schema.sql` 실행**
   - `supabase/migrations/setup_schema.sql` 파일 내용을 복사하여 붙여넣기
   - **"Run"** 클릭하여 실행
   - 성공 메시지 확인

   **5-2. `setup_clerk_integration.sql` 실행**
   - `supabase/migrations/setup_clerk_integration.sql` 파일 내용을 복사하여 붙여넣기
   - **"Run"** 클릭하여 실행
   - 성공 메시지 확인

> 💡 **로컬 개발 환경**: Supabase CLI를 사용하는 경우 `supabase db reset` 명령으로 모든 마이그레이션을 자동 실행할 수 있습니다.

**생성되는 테이블:**
- `users`: Clerk 사용자와 동기화되는 사용자 정보 테이블 (RLS 정책 포함)
- `tasks`: 예제 tasks 테이블 (Clerk user_id 기반 RLS 정책 포함)

> 💡 **참고**: 마이그레이션 파일은 두 개입니다:
> - `setup_schema.sql`: `users` 테이블 및 RLS 정책
> - `setup_clerk_integration.sql`: `tasks` 예제 테이블 및 RLS 정책
